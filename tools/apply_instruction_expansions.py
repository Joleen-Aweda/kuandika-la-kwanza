#!/usr/bin/env python3
"""Apply the reviewed dot-spacing instructions without duplicating exercises."""
...
EXPANSIONS = {
    ...
    "pg017_n0025": (17, "Andika herufi ya konsonanti b. Andika doti ya kwanza na ya pili kwa nafasi na ujaze mstari."),
    "pg021_n0008": (21, "Andika herufi ya konsonanti k. Andika doti ya kwanza na ya tatu kwa nafasi na ujaze mstari."),
    "pg023_n0002": (23, "Andika herufi ya konsonanti d. Andika doti ya kwanza, ya nne na ya tano kwa nafasi na ujaze mstari."),
    "pg024_n0031": (24, "Andika herufi ya konsonanti n. Andika doti ya kwanza, ya tatu, ya nne na ya tano kwa nafasi na ujaze mstari."),
    "pg029_n0011": (29, "Andika herufi ya konsonanti t. Andika doti ya pili, ya tatu, ya nne na ya tano kwa nafasi na ujaze mstari."),
    ...
    "pg039_n0018": (39, "Andika herufi ya konsonanti g. Andika doti ya kwanza, ya pili, ya nne na ya tano kwa nafasi na ujaze mstari."),
    ...
    "pg042_n0020": (42, "Andika herufi ya konsonanti z. Andika doti ya kwanza, ya tatu, ya tano na ya sita kwa nafasi na ujaze mstari."),
    "pg044_n0011": (44, "Andika herufi ya konsonanti h. Andika doti ya kwanza, ya pili na ya tano kwa nafasi na ujaze mstari."),
    "pg047_n0020": (47, "Andika herufi ya konsonanti r. Andika doti ya kwanza, ya pili, ya tatu na ya tano kwa nafasi na ujaze mstari."),
    ...
    "pg052_n0013": (52, "Andika herufi ya konsonanti ch. Andika doti ya kwanza na ya nne, na doti ya kwanza, ya pili na ya tano kwa nafasi na ujaze mstari."),
    "pg055_n0009": (55, "Andika herufi ya irabu A. Katika kuandika irabu A ijae katika mstari mdogo. Andika doti ya sita na ya kwanza kwa nafasi na ujaze mstari. Andika herufi kubwa A ikifuatiwa na herufi ndogo a kwa pamoja kisha, andika tena herufi hizo kwa nafasi na ujaze mstari."),
    ...
    "pg063_n0023": (63, "Andika herufi ya konsonanti B. Andika doti ya sita, ya kwanza na ya pili kwa nafasi na ujaze mstari. Andika herufi kubwa B ikifuatiwa na herufi ndogo b kwa pamoja kisha, andika tena herufi hizo kwa nafasi na ujaze mstari."),
...
def update_layout() -> None:
    path = ROOT / "content" / "guided-practice-layout.json"
    layout = json.loads(path.read_text(encoding="utf-8"))
    changed = 0
    for text_id, (page_number, value) in EXPANSIONS.items():
        page_id = f"pg{page_number:03}_sec001"
        original = value.split(". ", 1)[0] + "."
        matches = [
            entry
            for entry in layout[page_id]["entries"]
            if str(entry["text"]).strip() == original
            or str(entry["text"]).strip() == value
        ]
        if len(matches) != 1:
            raise RuntimeError(f"Expected one layout entry for {text_id}, found {len(matches)}")
        entry = matches[0]
        entry["text"] = value
        if text_id == "pg017_n0025":
            entry["domIndex"] = 2
        changed += 1
    path.write_text(
    ...
    if changed != len(EXPANSIONS):
        raise RuntimeError(f"Updated {changed} of {len(EXPANSIONS)} layout entries")
